...
# --- FUNÇÃO PARA ACESSAR O BANCO DE DADOS APÓS LOGIN ---
def get_colecao():
...
def tela_cadastro():
    ...
    if st.button("💾 Salvar Orçamento"):
        if not all([nome_cliente, endereco, cidade, cnpj]):
            ...
            get_colecao().insert_one(orcamento)
...
def tela_listagem():
    ...
    orcamentos = list(get_colecao().find(query).sort("data", -1))
    if not orcamentos:
    ...
    for o in orcamentos:
        with st.expander(f"{o['cliente']['nome']} - {o['data'].strftime('%d/%m/%Y %H:%M')}"):
            ...
            if st.button("🗑️ Excluir", key=str(o["_id"])):
                get_colecao().delete_one({"_id": ObjectId(o["_id"])})
