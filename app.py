...
def tela_cadastro():
    ...
    if "acessorios_temp" not in st.session_state:
        st.session_state["acessorios_temp"] = []

    with st.form("form_acessorio"):
        ...
        add_acessorio = st.form_submit_button("Adicionar Acessório")

        if add_acessorio and descricao_acessorio:
            st.session_state["acessorios_temp"].append({
                "descricao": descricao_acessorio,
                "quantidade": quantidade_acessorio,
                "valor": valor_acessorio
            })

    for i, acc in enumerate(st.session_state["acessorios_temp"]):
        st.markdown(f"**{i+1}. {acc['descricao']} | Qtd: {acc['quantidade']} | Valor: R$ {acc['valor']:.2f}**")
    ...
    if st.button("💾 Salvar Orçamento"):
        if not all([nome_cliente, endereco, cidade, cnpj]):
            ...
            orcamento = {
                ...
                "acessorios": st.session_state["acessorios_temp"],
            ...
            del st.session_state["acessorios_temp"]
